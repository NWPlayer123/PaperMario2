...
USERFUNC_DEF(evt_audience_flag_check) {
	...
	flags = evtGetValue(evt, args[1]);
	if (BattleAudienceGetPtr(id)->flags & flags) {
		evtSetValue(evt, args[2], TRUE);
	}
	else {
		evtSetValue(evt, args[2], FALSE);
	}
	return EVT_RETURN_DONE;
}
...
USERFUNC_DEF(evt_audience_get_position) {
	...
	evtSetFloat(evt, args[1], member->position.x);
	evtSetFloat(evt, args[2], member->position.y);
	evtSetFloat(evt, args[3], member->position.z);
	return EVT_RETURN_DONE;
}
...
USERFUNC_DEF(evt_audience_move_position_frame) {
	...
	id = evtGetValue(evt, args[0]);
	targetX = evtGetFloat(evt, args[1]);
	targetY = evtGetFloat(evt, args[2]);
	targetZ = evtGetFloat(evt, args[3]);
	mode = evtGetValue(evt, args[4]);
	frames = evtGetValue(evt, args[5]);
	member = BattleAudienceGetPtr(id);
	if (isFirstCall) {
...
USERFUNC_DEF(evt_audience_jump_position_firstsp) {
	...
	targetY = evtGetFloat(evt, args[2]);
	targetZ = evtGetFloat(evt, args[3]);
	frames = evtGetValue(evt, args[4]);
	deltaY = evtGetFloat(evt, args[5]);
	member = BattleAudienceGetPtr(id);
	if (isFirstCall) {
...
USERFUNC_DEF(evt_audience_jump_position_gravity) {
	...
	targetZ = evtGetFloat(evt, args[3]);
	frames = evtGetValue(evt, args[4]);
	gravity = evtGetFloat(evt, args[5]);
	member = BattleAudienceGetPtr(id);
	if (isFirstCall) {
		amountX = targetX - member->position.x;
...
USERFUNC_DEF(evt_audience_set_animpose) {
	...
	id = evtGetValue(evt, args[0]);
	animId = evtGetValue(evt, args[1]);
	force = evtGetValue(evt, args[2]);
...
USERFUNC_DEF(evt_audience_entry) {
	...
	args = evt->args;
	id = evtGetValue(evt, args[0]);
	type = (AudienceMemberType)evtGetValue(evt, args[1]);
...
USERFUNC_DEF(evt_audience_delete) {
	s32 id = evtGetValue(evt, *evt->args);
...
USERFUNC_DEF(evt_audience_reflesh_natural_all) {
	...
	chance = evtGetValue(evt, args[0]);
	evtGetValue(evt, args[1]); //unused
	evtGetValue(evt, args[2]); //unused
